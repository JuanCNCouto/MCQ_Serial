			...
			if( (ndp/dp[ i[1]+ j[1]*N ]) >= r) // condição de aceitação do novo valor
			{
				...
				for(c=i[1]-1; c <= i[1] +1 ;c++)
				{
					if(c>0 && c<(N-1))
					{
						...
						auxE= auxE - dEt[ c + j[1]*N]; //E0 - old et
						if(c==i[1])
							auxE = auxE + ndp*nEl1[index]*dx*dy ;
						else       
							auxE= dp[c +j[1]*N]*nEl1[index]*dx*dy ; // E0 + new_dEt
					}
					index= index +1;
				}	
				index=0;
				for(c= j[1] -1;c<=j[1]+1 ;c++)
				{
					if(c>0 && c<(N-1))
					{
						...
						if(c==j[1])
							auxE = (auxE + ndp*nEl2[index]*dx*dy  );
						else
							auxE= (dp[i[1] +c*N]*nEl2[index]*dx*dy );
					}
				...
				if(fabs(auxE)<fabs(Et[0])) //se a energia diminuir, atualiza os valores
				{
					index=0;
					dp[i[1]+j[1]*N] =ndp;
					Psi[ i[1] + j[1]*N ] = nPsi;
